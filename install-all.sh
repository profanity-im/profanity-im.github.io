...
debian_prepare()
{
    ...
    echo
    sudo apt-get -y install git automake autoconf libssl-dev libexpat1-dev libncursesw5-dev libglib2.0-dev libnotify-dev libcurl3-dev libxss-dev libotr5-dev libreadline-dev libtool libgpgme11-dev libgtk2.0-dev autoconf-archive python-dev
...
opensuse_prepare()
{
    ...
    echo Profanity installer...installing dependencies
    echo
    sudo zypper -n in gcc git automake make autoconf libopenssl-devel expat libexpat-devel ncurses-devel glib2-devel libnotify-devel libcurl-devel libXScrnSaver-devel libotr-devel readline-devel libtool libgpgme-devel gtk2-devel autoconf-archive python-devel
}
...
opensuse)   opensuse_prepare
            install_lib_mesode --prefix=/usr/local
            sudo /sbin/ldconfig
            install_profanity
            ;;
centos)     centos_prepare
            install_lib_mesode --libdir=/lib64
            sudo ldconfig
            install_profanity
            ;;
